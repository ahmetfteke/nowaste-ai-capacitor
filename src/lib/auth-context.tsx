...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signInWithApple = async () => {
    ...
    if (isCapacitor) {
      console.log("[Auth] Apple: starting native sign-in...");
      const { FirebaseAuthentication } = await import(
        "@capacitor-firebase/authentication"
      );
      const nativeResult = await FirebaseAuthentication.signInWithApple();
      console.log("[Auth] Apple: native result:", JSON.stringify(nativeResult, null, 2));

      const idToken = nativeResult.credential?.idToken;
      const rawNonce = nativeResult.credential?.nonce;
      console.log("[Auth] Apple: idToken exists:", !!idToken, "nonce exists:", !!rawNonce);
      ...
      console.log("[Auth] Apple: calling signInWithCredential...");
      try {
        result = await signInWithCredential(auth, credential);
        console.log("[Auth] Apple: signInWithCredential success, uid:", result.user.uid);
      } catch (credError: any) {
        console.error("[Auth] Apple: signInWithCredential FAILED:", credError?.code, credError?.message);
        throw credError;
      }
    } else {
    ...
    console.log("[Auth] Apple: checking if new user...");
    if (result.user.metadata.creationTime === result.user.metadata.lastSignInTime) {
      console.log("[Auth] Apple: new user, initializing...");
      await initializeNewUser(
        result.user.uid,
        result.user.email || "",
        result.user.displayName
    ...
    console.log("[Auth] Apple: sign-in complete, ready to route");
  };
